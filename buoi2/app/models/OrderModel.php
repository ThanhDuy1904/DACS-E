...
class OrderModel {
    ...
    public function getOrders() {
        try {
            $query = "SELECT o.id, o.user_id, o.payment_method, o.customer_name, o.phone, 
                             o.created_at, o.status, o.discount_code, o.discount_amount,
                             u.username 
                      FROM orders o 
    ...
    /**
     * Tạo đơn hàng mới với discount được phân bổ vào từng item
     */
    public function addOrderWithDetails($userId, $paymentMethod, $products, $customerName = null, $phone = null, $discountCode = null, $discountAmount = 0) {
        try {
            ...
            // Tính tổng tiền trước discount để phân bổ tỷ lệ
            ...
            // Tính tỷ lệ discount
            $discountRatio = ($totalBeforeDiscount > 0 && $discountAmount > 0) ? 
                            ($discountAmount / $totalBeforeDiscount) : 0;
            
            // Thêm chi tiết đơn hàng với giá đã discount
            $detailQuery = "INSERT INTO order_details (order_id, product_id, quantity, sugar_level, ice_level, cup_size, price, original_price) 
                           VALUES (:order_id, :product_id, :quantity, :sugar_level, :ice_level, :cup_size, :price, :original_price)";
            ...
            foreach ($products as $product) {
                ...
                // Tính giá sau discount cho từng item
                $discountedPrice = $originalPrice;
                if ($discountRatio > 0) {
                    $itemDiscountAmount = $originalPrice * $discountRatio;
                    $discountedPrice = round($originalPrice - $itemDiscountAmount, 2);
                }
                ...
                $detailStmt->bindValue(':price', $discountedPrice);
                $detailStmt->bindValue(':original_price', $originalPrice);
    ...
    public function addNewOrderDetails($orderId, $newProducts) {
        try {
            ...
            $query = "INSERT INTO order_details (order_id, product_id, quantity, sugar_level, ice_level, cup_size, price, original_price) 
                      VALUES (:order_id, :product_id, :quantity, :sugar_level, :ice_level, :cup_size, :price, :original_price)";
            ...
            foreach ($newProducts as $product) {
                ...
                $stmt->bindParam(':price', $product['price'], PDO::PARAM_STR);
                $stmt->bindParam(':original_price', $product['original_price'] ?? $product['price'], PDO::PARAM_STR);
                $stmt->execute();
            }
    ...
    public function getOrderStatistics() {
        try {
            ...
            // Tổng doanh thu (sử dụng giá đã discount)
            $query = "SELECT SUM(od.price * od.quantity) as total_revenue 
                      FROM order_details od";
            $stmt = $this->conn->prepare($query);
            $stmt->execute();
            ...
            // Doanh thu hôm nay
            $query = "SELECT SUM(od.price * od.quantity) as today_revenue 
                      FROM order_details od 
    ...
    /**
     * Áp dụng discount cho đơn hàng đã có
     */
    public function applyDiscount($orderId, $discountCode, $discountAmount) {
        try {
            ...
            $stmt = $this->conn->prepare("
                SELECT od.id, od.price, od.quantity, od.original_price
                FROM order_details od 
                WHERE od.order_id = ?
            ");
            ...
            // Tính tổng tiền từ giá gốc
            $totalAmount = 0;
            foreach ($details as $detail) {
                $originalPrice = $detail['original_price'] ?? $detail['price'];
                $totalAmount += ($originalPrice * $detail['quantity']);
            }
            
            // Tính tỷ lệ discount
            $discountRatio = ($totalAmount > 0) ? ($discountAmount / $totalAmount) : 0;
            
            // Cập nhật đơn hàng với thông tin discount
            $stmt = $this->conn->prepare("
                UPDATE orders 
                SET discount_code = ?, discount_amount = ? 
                WHERE id = ?
            ");
            $stmt->execute([$discountCode, $discountAmount, $orderId]);
            
            // Cập nhật giá trong order_details
            $updateStmt = $this->conn->prepare("
                UPDATE order_details 
                SET price = ROUND((original_price ?? price) * (1 - ?), 2),
                    original_price = COALESCE(original_price, price)
                WHERE id = ?
            ");
            
            foreach ($details as $detail) {
                $updateStmt->execute([$discountRatio, $detail['id']]);
            }
