...
class DashboardProductController {
    ...
    public function toggleVisibility() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['id'])) {
            ...
            $product = $this->getProductById($productId);
            if ($product) {
                $newVisibility = $product['is_visible'] ? 0 : 1; // Đảo ngược trạng thái hiển thị
                
                // Cập nhật trạng thái hiển thị sản phẩm
                $this->updateProductVisibility($productId, $newVisibility);
                
                $_SESSION['success_message'] = 'Trạng thái hiển thị của sản phẩm đã được cập nhật.';
            } else {
                $_SESSION['error_message'] = 'Không tìm thấy sản phẩm.';
            }
        }
    ...
    private function getAllProductsWithSales() {
        try {
            $query = "SELECT p.id, p.name, p.price, p.image, p.description, p.category_id,
                      ...
                      GROUP BY p.id, p.name, p.price, p.image, p.description, p.category_id, c.name
                      ORDER BY p.name ASC";
            ...
            error_log("Error in getAllProductsWithSales: " . $e->getMessage());
            return [];
        }
    }
    ...
    private function updateProductVisibility($productId, $isVisible) {
        try {
            $query = "UPDATE product SET is_visible = ? WHERE id = ?";
            $stmt = $this->conn->prepare($query);
            return $stmt->execute([$isVisible, $productId]);

        } catch (PDOException $e) {
            error_log("Error in updateProductVisibility: " . $e->getMessage());
            return false;
        }
    }
