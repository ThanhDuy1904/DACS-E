...
class OrderController {
    ...
    public function index() {
        ...
        // Lấy đơn hàng theo bộ lọc (bao gồm cancel_reason)
    ...
    // Trang chỉnh sửa đơn hàng - cho phép cả admin và employee xem chi tiết
    public function edit() {
        ...
        // Lấy danh sách sản phẩm để hiển thị trong form thêm sản phẩm (chỉ admin mới cần)
    ...
    // Xuất đơn hàng ra định dạng PDF
    public function exportToPdf($orderId) {
        ...
        // Thiết lập header cho file HTML (sẽ được in thành PDF)
        header("Content-Type: text/html; charset=utf-8");

        // Nạp template PDF
        require 'app/views/order/pdf_template.php';
        exit;
    }
    ...
    public function cancelOrder() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['order_id']) && isset($_POST['cancel_reason'])) {
            ...
            if (empty($cancelReason)) {
                ...
                $result = $this->orderModel->cancelOrder($orderId, $cancelReason);
